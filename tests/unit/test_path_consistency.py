...
"""Path-consistency invariant: two :class:`SandboxConfig` instances agree.
...
source of truth per path — :class:`SandboxConfig` — that every
...
#: Every public path property on :class:`SandboxConfig` that the
...
def test_two_configs_agree_on_every_path() -> None:
    """Two :class:`SandboxConfig` instances resolve every path identically.

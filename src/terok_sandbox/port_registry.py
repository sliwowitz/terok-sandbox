...
class PortRegistry:
    ...
    def claim(
        ...
        *,
        ...
        self._ensure_dir()
        others = self._read_all_claims() - set(self._held.values())
    ...
    def _read_all_claims(self) -> set[int]:
        """Read all claim files (including own), returning the set of taken ports.

        Own persisted claims from previous sessions are included so that
        concurrent same-user launches see each other's allocations.
        """
        taken: set[int] = set()
        try:

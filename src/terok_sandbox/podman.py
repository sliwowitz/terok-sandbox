...
Exposes [`PodmanInspector`][terok_sandbox.podman.PodmanInspector] (ID → [`ContainerInfo`][terok_clearance.ContainerInfo], with
...
The data type [`ContainerInfo`][terok_clearance.ContainerInfo] lives in terok-clearance because
...
class PodmanInspector:
    """Cached ID → [`ContainerInfo`][terok_clearance.ContainerInfo] lookup backed by ``podman inspect``.
    ...
    [`ContainerInfo`][terok_clearance.ContainerInfo], so callers keep a usable fallback.
...
def _from_inspect(container_id: str, records: Any) -> ContainerInfo:
    """Build a [`ContainerInfo`][terok_clearance.ContainerInfo] from a ``podman inspect`` JSON payload."""
...
def create_container_inspector() -> ContainerInspector:
    """Return a [`ContainerInspector`][terok_clearance.ContainerInspector] matched to the active runtime.
